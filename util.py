...
def scale(point, scale_factor, pivot):
    return [
        (point[0] - pivot[0]) * scale_factor + pivot[0],
        (point[1] - pivot[1]) * scale_factor + pivot[1],
    ]
...
def draw_pose_json(
    ...
    if pose_json:
        ...
        for image in images:
            ...
            for figure_idx, figure in enumerate(figures):
                ...
                if body:
                    ...
                    for i in range(0, len(body), 3):
                        ...
                        if body_scale != 1.0:
                            # Use the same pivot approach as other scaling operations
                            body_pivot = [W * 0.5, H * 0.5]  # Image center in pixels
                            point = scale(p, body_scale, body_pivot)
                        else:
                            point = p[:]
                    ...
                    if figure_head_idx < len(candidate):
                        face_offset = [
                            candidate[figure_head_idx][0] - body[0],
                            candidate[figure_head_idx][1] - body[1],
                        ]
                    ...
                    if wrist_left_idx < len(candidate) and len(body) > 22:
                        lhand_offset = [
                            candidate[wrist_left_idx][0] - body[21],
                            candidate[wrist_left_idx][1] - body[22],
                        ]
                    ...
                    if wrist_right_idx < len(candidate) and len(body) > 13:
                        rhand_offset = [
                            candidate[wrist_right_idx][0] - body[12],
                            candidate[wrist_right_idx][1] - body[13],
                        ]
                    ...
                    if not subset[0]:
                        ...
                        new_subset = [
                            candidate_start_idx + (i // 3) if body[i + 2] > 0 else -1
                            for i in range(0, len(body), 3)
                        ]
            ...
            if hands:
                hands = [
                    [scale(lm, overall_scale, normalized_pivot) for lm in hand]
                    for hand in hands
                ]
            if faces:
                faces = [
                    [scale(lm, overall_scale, normalized_pivot) for lm in face]
                    for face in faces
                ]
            if candidate:
                candidate = [
                    scale(lm, overall_scale, normalized_pivot) for lm in candidate
                ]
...
def draw_bodypose(canvas, candidate, subset, pose_marker_size):
    ...
    for i in range(18):
        for n in range(len(subset)):
            ...
            y = int(y * H)
            cv2.circle(
                canvas, (int(x), int(y)), pose_marker_size, colors[i], thickness=-1
            )
