        ...
        return {
            "optional": {
                ...
                "scalelist_behavior": (
                    ...
                    {
                        "default": "poses",
                        "tooltip": "When the scale input is a list, this determines how the scale list takes effect, the differences appear when there are multiple persons(poses) in one image.",
                    },
                ),
                "match_scalelist_method": (
                    ["no extend", "loop extend", "clamp extend"],
                    {
                        "default": "loop extend",
                        "tooltip": "Match the scale list to the input poses or images when the scale list length is shorter. No extend: Beyound the scale list will be 1.0. Loop: Loop the scale list to match the poses or images length. Clamp: Use the last scale value to extend the scale list.",
                    },
                ),
    ...
    def load_pose(
        ...
        if POSE_JSON:
            try:
                ...
                json.loads(POSE_JSON if POSE_JSON.startswith("[") else f"[{POSE_JSON}]")
                POSE_PASS = POSE_JSON
        ...
        if POSE_JSON:
            if POSE_KEYPOINT is not None:
                try:
                    POSE_PASS = (
                        json.dumps(POSE_KEYPOINT, indent=4)
                        .replace("'", '"')
                        .replace("None", "[]")
                    )
                    (
                    ...
                    pose_imgs, POSE_PASS = draw_pose_json(
                        POSE_PASS,
                        resolution_x,
                        show_body,
                        show_face,
            ...
            try:
                ...
                pose_imgs, POSE_JSON_SCALED = draw_pose_json(
                    POSE_JSON,
                    resolution_x,
                    show_body,
                    show_face,
                ...
                if pose_imgs:
                    ...
                    return {
                        "ui": {
                            "POSE_JSON": [
                                json.dumps(
                                    POSE_PASS if POSE_PASS else POSE_JSON_SCALED,
                                    indent=4,
                                )
                            ]
                        },
                        "result": (
                            ...
                            POSE_JSON_SCALED,
                            json.dumps(POSE_JSON_SCALED, indent=4),
                        ),
                    }
            except Exception as e:
                ...
                if pose_imgs:
                    ...
                    return {
                        ...
                        "result": (
                            torch.from_numpy(pose_imgs_np),
                            POSE_SCALED,
                            json.dumps(POSE_SCALED, indent=4),
                        ),
                    }
            except Exception as e:
                print(f"Error processing POSE_KEYPOINT: {e}")
        ...
        try:
            ...
            return {
                ...
                "result": (
                    torch.from_numpy(blank_img[None, ...]),
                    people,
                    json.dumps(people),
                ),
            }
